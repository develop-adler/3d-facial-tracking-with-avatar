...
// Original authors from Babylon.js team. Adapted by T3 Chat. Heavily updated and fixed by Gavin Quach.
...
// --- TYPE DEFINITIONS FOR JS PHYSICS INTEGRATION ---
// These interfaces define the contract between the plugin and the physics objects.
// It's assumed that the `PhysicsBody` is attached to a Object3D's `userData`.
/**
...
// Helper class to gather mesh data from a js hierarchy
class MeshAccumulator {
    ...
    public addNodeMeshes(mesh: Object3D, includeChildren: boolean): void {
        mesh.updateWorldMatrix(true, false);
        const rootDecomposition = {
            scale: new Vector3(),
            position: new Vector3(),
            quaternion: new Quaternion(),
        };
        mesh.matrixWorld.decompose(
            rootDecomposition.position,
            rootDecomposition.quaternion,
            rootDecomposition.scale
        );

        const rootScaled = new Matrix4().makeScale(
            rootDecomposition.scale.x,
            rootDecomposition.scale.y,
            rootDecomposition.scale.z
        );

        if ((mesh as Mesh).isMesh) {
            this._addMesh(mesh as Mesh, rootScaled);
        } else if ((mesh as InstancedMesh).isInstancedMesh) {
            this._addMesh((mesh as InstancedMesh).userData.sourceMesh, rootScaled);
        }

        if (includeChildren) {
            const worldToRoot = new Matrix4();
            mesh.updateWorldMatrix(true, false);
            worldToRoot.copy(mesh.matrixWorld).invert();

            const worldToRootScaled = new Matrix4().multiplyMatrices(
                worldToRoot,
                rootScaled
            );

            const children = mesh.children.filter(
                (m) => !(m.userData && m.userData.physicsBody)
            );

            for (const m of children) {
                m.updateWorldMatrix(true, false);
                const childToWorld = m.matrixWorld;
                const childToRootScaled = new Matrix4().multiplyMatrices(
                    childToWorld,
                    worldToRootScaled
                );
                ...
                } else if ((m as InstancedMesh).isInstancedMesh) {
                    this._addMesh(
                        (m as InstancedMesh).userData.sourceMesh,
                        childToRootScaled
                    );
                }
            }
    ...
    private _addMesh(mesh: Mesh, meshToRoot: Matrix4): void {
        const vertexData = mesh.geometry.attributes.position;
        if (!vertexData) {
            return;
        }
        const numVerts = vertexData.count;
        const indexOffset = this._vertices.length;

        for (let v = 0; v < numVerts; v++) {
            const pos = new Vector3().fromBufferAttribute(vertexData, v);
            this._vertices.push(pos.applyMatrix4(meshToRoot));
        }

        if (this._collectIndices) {
            const meshIndices = mesh.geometry.index;
            if (meshIndices) {
                for (let i = 0; i < meshIndices.count; i += 3) {
                    // js is right-handed, so winding order is correct by default
                    this._indices.push(
                        meshIndices.getX(i + 0) + indexOffset,
                        meshIndices.getX(i + 1) + indexOffset,
                        meshIndices.getX(i + 2) + indexOffset
                    );
                }
            }
        }
    ...
    public getVertices(plugin: any): { offset: number; numObjects: number } {
    ...
    public freeBuffer(plugin: any, arr: { offset: number }): void {
        plugin._free(arr.offset);
    }

    public getTriangles(plugin: any): { offset: number; numObjects: number } {
        const bytesPerInt = 4;
        const nBytes = this._indices.length * bytesPerInt;
        const bufferBegin = plugin._malloc(nBytes);
        const ret = new Int32Array(
            plugin.HEAPU8.buffer,
            bufferBegin,
 ...
 */
export class HavokPlugin extends EventDispatcher {
    ...
    private _bodies: Map<number, { body: PhysicsBody; index: number }> =
        new Map();
    private _shapes: Map<number, PhysicsShape> = new Map();
    private _constraintToBodyIdPair: Map<bigint, [number, number]> = new Map();
    private _queryCollector: any;
    private _bodyBuffer: any;
    ...
    public executeStep(delta: number, physicsBodies: PhysicsBody[]): void {
    ...
    public getShapeType(shape: PhysicsShape): PhysicsShapeType {
        ...
        return shape.type ?? this._hknp.HP_Shape_GetType(shape._pluginData) as unknown as PhysicsShapeType;
    }
    ...
    private _internalUpdateMassProperties(pluginData: BodyPluginData): void {
        ...
        this._hknp.HP_Body_SetMassProperties(pluginData.hpBodyId, newProps as MassProperties);
    }
     ...
     *
     */
    addChild(shape: PhysicsShape, newChild: PhysicsShape, translation: Vector3, rotation: Quaternion, scale: Vector3) {
        const transformNative: QSTransform = [
            translation ? [translation.x, translation.y, translation.z] : [0, 0, 0],
            rotation
                ? [rotation.x, rotation.y, rotation.z, rotation.w]
                : [0, 0, 0, 1],
            scale ? [scale.x, scale.y, scale.z] : [1, 1, 1],
        ];
     ...
     *
     */
    setAxisFriction(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, friction: number) {
        for (const jointId of constraint._pluginData) {
     ...
     */
    setAxisMode(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, limitMode: PhysicsConstraintAxisLimitMode) {
        for (const jointId of constraint._pluginData) {
            this._hknp.HP_Constraint_SetAxisMode(
                jointId,
                this._constraintAxisToNative(axis),
     ...
     *
     */
    setAxisMinLimit(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, limit: number) {
        for (const jointId of constraint._pluginData) {
     ...
     *
     */
    setAxisMaxLimit(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, limit: number) {
        for (const jointId of constraint._pluginData) {
     ...
     *
     */
    setAxisMotorType(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, motorType: PhysicsConstraintMotorType) {
        for (const jointId of constraint._pluginData) {
     ...
     *
     */
    setAxisMotorTarget(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, target: number) {
        for (const jointId of constraint._pluginData) {
     ...
     *
     */
    getAxisMotorTarget(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis) {
        const firstId = constraint._pluginData && constraint._pluginData[0];
        if (firstId) {
     ...
     *
     */
    setAxisMotorMaxForce(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis, maxForce: number) {
        for (const jointId of constraint._pluginData) {
     ...
     *
     */
    getAxisMotorMaxForce(constraint: PhysicsConstraint, axis: PhysicsConstraintAxis) {
        const firstId = constraint._pluginData && constraint._pluginData[0];
        if (firstId) {
    ...
    public raycast(
        from: Vector3,
        to: Vector3,
        result: any, // PhysicsRaycastResult
        query?: any // IRaycastQuery
    ): void {
        ...
        const hkQuery: RayCastInput = [
            this._threeVectorToHavokArray(from),
            this._threeVectorToHavokArray(to),
    ...
    private _notifyCollisions(): void {
        ...
        while (eventAddress) {
            ...
            const bodyInfoA = this._bodies.get(event.contactOnA.bodyId as unknown as number);
            const bodyInfoB = this._bodies.get(event.contactOnB.bodyId as unknown as number);
            ...
            if (bodyInfoA && bodyInfoB) {
                ...
                if (collisionInfo.type === "COLLISION_FINISHED") {
                    ...
                    this.dispatchEvent({
                        type: "collisionended",
                        detail: collisionInfo,
                    });
                } else {
                    event.contactOnB.position.sub(event.contactOnA.position);
            ...
            eventAddress = this._hknp.HP_World_GetNextCollisionEvent(
                worldAddr,
                eventAddress
            );
        }
    ...
    private _notifyTriggers(): void {
        ...
        while (eventAddress) {
            TriggerEvent.readToRef(this._hknp.HEAPU8.buffer as ArrayBuffer, eventAddress, event);
            const bodyInfoA = this._bodies.get(event.bodyIdA as unknown as number);
            const bodyInfoB = this._bodies.get(event.bodyIdB as unknown as number);
            if (bodyInfoA && bodyInfoB) {
