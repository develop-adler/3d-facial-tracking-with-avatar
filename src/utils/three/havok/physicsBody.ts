...
import type { HavokPlugin, PhysicsConstraint, PhysicsMassProperties } from "./havokPlugin";
...
export class PhysicsBody {
    ...
    updateBodyInstances() {
        const m = this.node;
        if (m.count) {
            this._physicsPlugin.updateBodyInstances(
                this,
                m as InstancedMesh
            );
        }
    ...
    setEventMask(eventMask: number, instanceIndex: number) {
        this._physicsPlugin.setEventMask(
            this,
            eventMask,
            instanceIndex
        );
    ...
    getEventMask(instanceIndex: number) {
        return this._physicsPlugin.getEventMask(
            this,
            instanceIndex
        );
    }
    ...
    setMotionType(motionType: PhysicsMotionType, instanceIndex: number) {
        this.disableSync = motionType == 0 /* PhysicsMotionType.STATIC */;
        this._physicsPlugin.setMotionType(
            this,
            motionType,
            instanceIndex
        );
    ...
    getMotionType(instanceIndex?: number) {
        return this._physicsPlugin.getMotionType(
            this,
            instanceIndex
        );
    }
    ...
    computeMassProperties(instanceIndex: number) {
        return this._physicsPlugin.computeMassProperties(
            this,
            instanceIndex
        );
    }
    ...
    setMassProperties(massProps: PhysicsMassProperties, instanceIndex?: number) {
        this._physicsPlugin.setMassProperties(
            this,
            massProps,
            instanceIndex
        );
    ...
    getMassProperties(instanceIndex?: number) {
        return this._physicsPlugin.getMassProperties(
            this,
            instanceIndex
        );
    }
    ...
    setLinearDamping(damping: number, instanceIndex?: number) {
        this._physicsPlugin.setLinearDamping(
            this,
            damping,
            instanceIndex
        );
    ...
    getLinearDamping(instanceIndex?: number) {
        return this._physicsPlugin.getLinearDamping(
            this,
            instanceIndex
        );
    }
    ...
    setAngularDamping(damping: number, instanceIndex?: number) {
        this._physicsPlugin.setAngularDamping(
            this,
            damping,
            instanceIndex
        );
    ...
    getAngularDamping(instanceIndex?: number) {
        return this._physicsPlugin.getAngularDamping(
            this,
            instanceIndex
        );
    }
    ...
    setLinearVelocity(linVel: Vector3, instanceIndex: number) {
        this._physicsPlugin.setLinearVelocity(
            this,
            linVel,
            instanceIndex
        );
    ...
    getLinearVelocityToRef(linVel: Vector3, instanceIndex: number) {
        this._physicsPlugin.getLinearVelocityToRef(
            this,
            linVel,
            instanceIndex
        );
    ...
    setAngularVelocity(angVel: Vector3, instanceIndex: number) {
        this._physicsPlugin.setAngularVelocity(
            this,
            angVel,
            instanceIndex
        );
    ...
    getAngularVelocityToRef(angVel: Vector3, instanceIndex?: number) {
        this._physicsPlugin.getAngularVelocityToRef(
            this,
            angVel,
            instanceIndex
        );
    ...
    applyImpulse(impulse: Vector3, location: Vector3, instanceIndex?: number) {
        this._physicsPlugin.applyImpulse(
            this,
            impulse,
            location,
            instanceIndex
    ...
    applyForce(force: Vector3, location: Vector3, instanceIndex: number) {
        this._physicsPlugin.applyForce(
            this,
            force,
            location,
            instanceIndex
    ...
    //     this._physicsPlugin.setCollisionCallbackEnabled(
    //         this,
    //         enabled
    //     );
    // }
    ...
    //     this._physicsPlugin.setCollisionEndedCallbackEnabled(
    //         this,
    //         enabled
    //     );
    // }
     ...
     * @returns geometric center of the associated mesh
     */
    getObjectCenterWorldToRef(ref: Vector3, _instanceIndex?: number) {
        if (this._pluginDataInstances?.length > 0) {
            // const index = instanceIndex ?? 0;
            // const matrixData = this.node._thinInstanceDataStorage.matrixData;
            // if (matrixData) {
            //     ref.set(
     ...
     */
    syncWithBone(
        bone: Bone, boneMesh: Mesh, jointPivot: Vector3, distToJoint?: number, adjustRotation?: Quaternion, boneAxis?: Vector3
    ) {
        const mesh = this.node;
        if (adjustRotation) {
            const tempQuat = new Quaternion();
            bone.getWorldQuaternion(tempQuat);
            mesh.quaternion.copy(tempQuat.multiply(adjustRotation));
        } else {
            mesh.applyQuaternion(bone.quaternion);
        }
        if (!boneAxis) {
        ...
        bone.getWorldDirection(boneDir).multiply(boneAxis);
    ...
    setGravityFactor(factor: number, instanceIndex: number) {
        this._physicsPlugin.setGravityFactor(
            this,
            factor,
            instanceIndex
        );
    ...
    getGravityFactor(instanceIndex: number) {
        return this._physicsPlugin.getGravityFactor(
            this,
            instanceIndex
        );
    }
     ...
     */
    setTargetTransform(position: Vector3, rotation: Quaternion, instanceIndex: number) {
    ...
    dispose() {
        ...
        // }
        if (Object.hasOwn(this.node.userData, "physicsBody")) {
